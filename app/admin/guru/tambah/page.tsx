...
export default function TambahGuruPage() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      const usernameFinal = form.username || form.nip;
      ...
      if (!result.success) {
        setErrorMsg(result.message || "Gagal menambah guru");
  ...
  return (
    <div className="min-h-screen bg-[#F5F3EE] px-8 py-8">
      ...
      <div className="max-w-4xl">
        <h1 className="text-2xl font-bold text-[#0F1B33]">Tambah Guru Baru</h1>
        <p className="text-sm text-[#6B7280] mt-1 mb-8">Form data guru</p>
        ...
        <form onSubmit={handleSubmit} className="flex gap-10 flex-wrap">
          ...
          <div className="flex-1 min-w-[240px] space-y-5">
            ...
            <Field label="NIP" required>
              <input required placeholder="Masukkan NIP Anda" value={form.nip}
                onChange={(e) => setForm({ ...form, nip: e.target.value })}
                className="w-full px-3 py-2.5 rounded-lg border border-[#D8D3C8] bg-white text-sm focus:outline-none focus:ring-2 focus:ring-[#C6992F]" />
            </Field>

            <Field label="Mata Pelajaran" required>
              <input required placeholder="Contoh: Matematika, Fisika" value={form.mataPelajaran}
          ...
          <div className="flex-1 min-w-[240px] space-y-5">
            <Field label="No. Telepon" required>
              <input required placeholder="Masukkan No Telepon" value={form.noTelepon}
            ...
            <Field label="Alamat" required>
              <textarea required rows={3} placeholder="Masukkan Alamat Lengkap" value={form.alamat}
            ...
            <Field label="Username (opsional, default: NIP)">
              <input placeholder="Kosongkan jika ingin memakai NIP" value={form.username}
